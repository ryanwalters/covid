...
const Graphs = ({ graphs, width }) => {
  ...
  return graphs.map(({ heading, subHeading, yLabel, positive, hospitalized, death, positivityRate, dateChecked }) => {
    const positivityPercentage = parseFloat(positivityRate * 100).toFixed(2);
      ...
      <Fragment key={heading}>
        ...
        {positivityRate && dateChecked && (
          <div className="font-weight-light">
            Positivity rate for {dayjs(dateChecked).format('MMM D')}:
            <span
              className={classnames(
                'lead font-weight-bold ml-1 d-inline-flex align-items-center',
                positivityPercentage > 5 ? 'text-warning' : 'text-success'
              )}
            >
              {`${positivityPercentage}%`}
              <FontAwesomeIcon icon={faQuestionCircle} className="ml-2 text-light" size="xs" id="positivityQuestion" />
